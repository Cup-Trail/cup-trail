    ...
    <div
      ...
      onTouchMove={e => {
        ...
        if (id && id.startsWith('star-handle')) {
          const newRating = parseFloat((element as HTMLDivElement).dataset.rating ?? "2");
          setPreviewRating(newRating);
        }
      }}
    >
    <div className="text-transparent pointer-events-none">★★★★★</div>
    <div className={`${isPreview ? 'text-amber-200' : 'text-amber-300'} absolute top-0 left-0 overflow-clip pointer-events-none`} style={{width: `${(isPreview ? previewRating : rating)/5*100}%` }}>★★★★★</div>
    <div className={`absolute top-0 w-full flex flex-row ${isPreview && 'cursor-pointer'}`}>
            <div
          ...
          {Array.from({length: Math.floor(MAX_RATING / RATING_INCREMENTS)}).map((_, i, arr) => {
      ...
      <div
        className='relative mx-auto xs:ml-0 text-5xl xs:text-4xl select-none group text-stroke cursor-pointer'
      >
      </div>
    </div>
  </div>
