...
export default function InsertReviewPage() {
  ...
  async function handleSubmit() {
    ...
    if (Number.isNaN(parsed) || parsed < 0 || parsed > 10) {
      setSnack({ open: true, message: 'Please enter a rating between 0 and 10.', severity: 'error' });
      return;
    }
    if (!review) {
  ...
  return (
    <Stack gap={2}>
      <Typography variant="h5" textAlign="center" fontWeight={700}>Add Your Review</Typography>
      <Typography variant="body2" textAlign="center" color="text.secondary">Track your favorite drinks from each shop!</Typography>
      ...
      <TextField label="Rating (0 - 10)" value={rating} onChange={(e) => setRating(e.target.value)} fullWidth />
      <TextField label="Your Review" value={review} onChange={(e) => setReview(e.target.value)} fullWidth multiline minRows={4} />
